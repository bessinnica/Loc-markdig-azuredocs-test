...
The “non-polling methods” have the following naming pattern: Send<em>OperationName</em>Operation (for example, SendCreateOperation). Send<em>OperationName</em>Operation methods return the <strong>IOperation</strong> object; the returned object contains information that can be used to track the operation. The Send<em>OperationName</em>OperationAsync methods return <strong>Task<IOperation></strong>.
