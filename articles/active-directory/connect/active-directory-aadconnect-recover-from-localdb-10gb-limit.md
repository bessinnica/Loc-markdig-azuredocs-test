...
4. You can either choose <strong>Clear all runs</strong> or <strong>Clear runs before… <date></strong> option. It is recommended that you start by clearing run history data that are older than two days. If you continue to run into DB size issue, then choose the <strong>Clear all runs</strong> option.
