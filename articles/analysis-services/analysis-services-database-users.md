...
3. Select a permission.
   ...
   |                  Permission                   |                                                Description                                                |
   |-----------------------------------------------|-----------------------------------------------------------------------------------------------------------|
   | <strong>Full control (Administrator)</strong> |                   Members can modify the model schema, process, and can query all data.                   |
   |       <strong>Process database</strong>       | Members can run Process and Process All operations. Cannot modify the model schema and cannot query data. |
   |             <strong>Read</strong>             |             Members can query data (based on row filters) but cannot modify the model schema.             |
