...
Now fetch the list of datasources on ```$server``` using the [Get-DPMDatasource](https://technet.microsoft.com/library/hh881605) cmdlet. In this example we are filtering for the volume <em>D:\* that we want to configure for backup. This datasource is then added to the Protection Group using the <a href="https://technet.microsoft.com/library/hh881732" data-raw-source="[Add-DPMChildDatasource](https://technet.microsoft.com/library/hh881732)">Add-DPMChildDatasource</a> cmdlet. Remember to use the *modifiable</em> protection group object ```$MPG``` to make the additions.
