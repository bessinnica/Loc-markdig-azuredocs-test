...
Downloading an encrypted blob involves retrieving the content of the entire blob using the <strong>download*/openInputStream</strong> convenience methods. The wrapped CEK is unwrapped and used together with the IV (stored as blob metadata in this case) to return the decrypted data to the users.
