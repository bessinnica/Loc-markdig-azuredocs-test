...
This article describes how to use the Service Fabric solution in Log Analytics to help identify and troubleshoot issues across your Service Fabric cluster. It helps you see how your Service Fabric nodes are performing and how your applications and micro-services are running.

The Service Fabric solution uses Azure Diagnostics data from your Service Fabric VMs, by collecting this data from your Azure WAD tables. Log Analytics then reads the following Service Fabric framework events:
...
1. Associate the Azure subscription that you used to create all cluster resources, including storage accounts, with your workspace. See [Get started with Log Analytics](log-analytics-get-started.md) for information about creating a Log Analytics workspace.
...
In this section, you learn how to configure Log Analytics to retrieve Service Fabric logs. The logs allow you to view, analyze, and troubleshoot issues in your cluster or in the applications and services running in that cluster, using the Azure portal.

> [!NOTE]
> Configure the Azure Diagnostics extension to upload the logs for storage tables. The tables must match what Log Analytics looks for. For more information, see [How to collect logs with Azure Diagnostics](../service-fabric/service-fabric-diagnostics-how-to-setup-wad.md). The configuration settings examples in this article show you what the names of the storage tables should be. Once Diagnostics is set up on the cluster and is uploading logs to a storage account, the next step is to configure Log Analytics to collect these logs.
>
>

Ensure that you update the **EtwEventSourceProviderConfiguration** section in the **template.json** file to add entries for the new EventSources before you apply the configuration update by running **deploy.ps1**. The table for upload is the same as (ETWEventTable). At the moment, Log Analytics can only read application ETW events from the *WADETWEventTable* table.
...
After you create a Log Analytics workspace, configure the workspace to pull logs from the Azure storage tables. Then, run the following PowerShell script:
...
After you've configured the Log Analytics workspace to read from the Azure tables in your storage account, log in to the Azure portal. Select the Log Analytics workspace from **All Resources**. The number of storage account logs connected to the workspace is displayed. Select the **Storage account logs** tile. Review the list of storage account logs to verify that your storage account is connected to the correct workspace.

![Storage account logs](./media/log-analytics-service-fabric/sf1.png)

## Enable the Service Fabric solution
Use the following script to add the solution to your Log Analytics workspace. Run the script in PowerShell, using the Azure subscription that is associated with the Log Analytics workspace that you want to enable the Service Fabric solution in.
...
After you enable the solution, the Service Fabric tile is added to your Log Analytics *Overview* page. The page shows a view of notable issues such as runAsync failures and cancellations that occurred in the last 24 hours.

![Service Fabric tile](./media/log-analytics-service-fabric/sf2.png)

### View Service Fabric events
Click the **Service Fabric** tile to open the Service Fabric dashboard. The dashboard includes the columns in the table that follows. Each column lists the top 10 events by count matching that column's criteria for the specified time range. You can run a log search that provides the entire list by clicking **See all** at the right bottom of each column, or by clicking the column header.

| **Service Fabric event** | **description** |
| --- | --- |
| Notable Issues | Displays issues including RunAsyncFailures, RunAsynCancellations, and Node Downs. |
| Operational Events | Displays notable operational events including application upgrade and deployments. |
| Reliable Service Events | Displays notable reliable service events including  Runasyncinvocations. |
| Actor Events | Displays notable actor events generated by your micro-services. Events include exceptions thrown by an actor method, actor activations and deactivations, and so on. |
| Application Events | Displays all custom ETW events generated by your applications. |
...
> [!NOTE]
> Change the scope of events with **Data based on last seven days** at the top of the dashboard. You can also show events generated within the last seven days, one day, or six hours. Or, you can select **Custom** to specify a custom date range.
>
>

## Troubleshoot your Service Fabric and Log Analytics configuration
If you need to verify your Log Analytics configuration because you are unable to view event data in Log Analytics, use the following script. It performs the following actions:

1. Reads your Service Fabric diagnostics configuration
2. Checks for data written into the tables
3. Verifies that Log Analytics is configured to read from the tables
