...
Navigate back to your storage account by clicking <strong>Storage</strong> > <strong>Storage Accounts</strong>, select your storage account and click <strong>Custom domain</strong>. Type in the alias you created without the asverify prefix in the text box, check <strong>Use indirect CNAME validation, and click **Save</strong>. Once this step is complete, return to your DNS zone and create a CNAME record without the asverify prefix.  After that point, you are safe to delete the CNAME record with the cdnverify prefix.
